...
**NOTE**: this role is probably no longer compatible with older versions of gitea due to removal of the deprecated `ISSUE_INDEXER_QUEUE_DIR` configuration.
...
### Queue ([queue](https://docs.gitea.io/en-us/config-cheat-sheet/#queue))
| variable name | default value | description |
| ------------- | ------------- | ----------- |
| `gitea_queue_extra_config` | `''` | you can use this variable to pass additional config parameters in the `[queue]` section of the config. |
